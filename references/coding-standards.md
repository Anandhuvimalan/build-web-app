...
- **Independent research/investigation** — "how does the current codebase handle X," "what does library Y's current API look like," "what's the existing test coverage for Z" have no dependency on each other and can run as parallel read-only agents whose findings you synthesize yourself before deciding anything. This doubles as the main context-hygiene tool (Phase 12, `methodology.md`): the agent spends *its* context on the raw files and long outputs, and the orchestrating session keeps only the distilled finding — offload bulk reading by default once a session is past its opening stretch.
...
- For slices with a UI surface, the end-to-end step expands to the **UI verification checklist in `frontend-design.md`**: multiple viewports, all four content states (empty/loading/error/populated), a throttled-network and throttled-CPU pass, zero layout shift, the transform/opacity animation rule, `prefers-reduced-motion`, keyboard-only operation, contrast/touch targets, draft protection on long forms, and conformance to `docs/DESIGN.md` tokens — plus the **Render QA pass** (`frontend-design.md`, Render & Motion Correctness): an eight-step reproduction script run in a real browser for first-paint flash, mid-page reloads, scroll-direction correctness, scrollbar/layout stability during animation, interruptibility, resize-during-animation, navigation cleanup, and moving-text legibility. It is the UI's version of "I ran it and watched it work," and it is not optional. For a *major* UI surface (nav, hero/intro, a page section), Done additionally requires its **design checkpoint** (`frontend-design.md`, Cinematic Direction): the user saw the built section, answered "is the direction right / what changes," and the feedback was applied or recorded as a standing decision — a surface the user never reviewed is not Done.
