...
| `references/frontend-design.md` (Book 4, extends Phases 1, 3, 9) | Anything UI: running design discovery (including deriving the project's own design concept — never a template default), choosing a layout system (bento grid or otherwise), extracting a reference site the user likes, computing container/spacing geometry, designing a small component (dropdowns, buttons, tooltips), designing a screen or admin workflow (quick-add, bulk-add), choreographing motion (parent/child stagger, a first-visit intro, scroll-triggered moments), or verifying a UI slice. Skip it for pure backend/schema slices. |
